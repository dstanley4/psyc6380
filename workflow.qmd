...
The data files below are used in this chapter. Be sure to download script_workflow.R and data_item_scoring.csv.
...
| [script_workflow.R](script_workflow.R)         |
...
It may seem that reproducibility should be a given - but it's not. Correspondingly, there is a trend for journals and authors to adopt Transparency and Openness Promotion (TOP) [guidelines](https://www.cos.io/our-services/top-guidelines). These guidelines involve such things as making your materials, data, code, and analysis scripts available on public repositories so anyone can check your data. A new open science journal rating system has even emerged called the [TOP Factor](https://topfactor.org).
...
The TOP open science guidelines have made an impact and some newer journals, such as Meta Psychology, have fully embraced open science. @fig-metapsychology shows the header from an [article](https://open.lnu.se/index.php/metapsychology/article/view/1630/2266) in Meta Psychology that clearly delineates the open science attributes of the article that used computer simulations (instead of participant data). Take note that the header even indicates who verified that the analyses in the article were reproducible.

```{r}
#| label: fig-metapsychology
#| echo: false
#| out-width: "100%"
#| fig-cap: "Open science in an article header"

knitr::include_graphics("ch_workflow/images/screenshot_metapsychology.png")
```
...
#| echo: false
#| out-width: "85%"
#| fig-cap: "Data science pipeline by Roger Peng"
...
```{r}
#| label: tbl-withintidyex
#| echo: false
#| tbl-cap: "A tidy data version of the within participant data"

knitr::kable(
  head(data_within_tidy,18),
  booktabs = TRUE,
  linesep = c("", "","\\addlinespace")
)
```
...
As your projects grow more complex, organizing your files into a clear directory structure becomes important. A recommended approach is to separate your data by its stage of processing:

```         
project/
├── script_analysis.R       # Your analysis script
├── data-raw/               # Original, untouched data files
├── data-processed/         # Final data ready for analysis
└── output/                 # Tables, figures, reports
```

-   **data-raw/** contains your original data exactly as you received it. Never modify files in this folder.
-   **data-processed/** contains data that has been cleaned and prepared for analysis.
-   **output/** contains any files your analysis produces (tables, figures, etc.).

In this chapter, we'll keep things simple with a single script. In a future lecture, we'll explore how to break your workflow into multiple scripts for larger projects.
...
Best practice is to create a unique participant identification number immediately after import. This allows you to track rows even if the order changes during processing. Note that the original data also has an id column. We create our own participant_id column and remove the original id column to avoid confusion.
...
-   `row_number()` generates sequential integers (1, 2, 3, ...) based on the current row position. Each participant gets a unique number.
-   `relocate()` moves columns to a new position. By default, it moves the specified column(s) to the first position. You can also use `.after` or `.before` arguments to place columns elsewhere (e.g., `relocate(participant_id, .after = sex)`).
-   `select(-id)` removes the original id column since we now have our own participant_id.
...
We have two variables, sex and eye_color, that are categorical variables of type character (i.e., chr). You can quickly convert character columns to factors using the code below:
...
**Note:** If you have factors like sex that have numeric data in the column (e.g, 1 and 2) instead of male/female you need to handle the situation differently. You would first convert the column to a factor and then use the fct_recode() command from the forcats package to change the numeric levels to text levels
...
It's also good practice to add a safety check that warns you if unexpected levels appear in your data. This can catch data entry errors or unexpected responses early.
...
The `setdiff()` function finds elements that are in the first set but not in the second. Here, it identifies any factor levels in our data that we didn't expect—which would indicate a data entry error or unexpected response.
...
::: callout-note
This reverse-coding approach works because our data file already contains numeric values (1, 2, 3, 4, 5). When working with Qualtrics survey exports, you'll typically receive text labels (e.g., "Strongly Disagree") instead of numbers. In the Qualtrics chapter, you'll learn how to convert those text labels to numbers before applying this reverse-coding technique.
:::
...
Once your data is prepared for analysis, it's good practice to save it. We use the `.rds` format because it preserves all R data types (factors, dates, etc.) exactly as they are. CSV files, by contrast, convert everything to text and lose this information.
...
Later, you can reload this data with `read_rds()`:

```{r}
#| eval: false

analytic_data_survey <- read_rds("data-processed/analytic_data_survey.rds")
```
