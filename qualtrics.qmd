...
Designing a survey process in way that ensure a high response rate is crucial to obtaining quality data. A low response rate can be problematic for many reasons. I strongly encourage you to check out the resource below to help you design a survey process that maximizes the response rate you obtain.

[Internet, Phone, Mail, and Mixed-Mode Surveys: The Tailored Design Method](https://a.co/d/fjk0WEG)
...
This section builds on the workflow principles from the previous chapter. You'll see familiar patterns—participant ID creation, factor handling, reverse coding, and scale creation—applied to Qualtrics data. You'll also learn new techniques specific to Qualtrics: handling the extra header rows, removing metadata columns, and converting text responses (like "Strongly Agree") to numbers.
...
In many analyses we will need a column with a unique identification number. We don't see one in this data set so we create it with the name participant_id. This is the same approach we used in the workflow chapter—`row_number()` generates sequential IDs and `relocate()` moves the column to the first position. (See the workflow chapter for detailed explanations of these functions.)
...
You can see the new participant_id column with the glimpse() command.
...
As discussed in the workflow chapter, it's good practice to add a safety check for unexpected factor levels using `setdiff()`:
...
To convert text labels to numbers, we first check what labels exist in our data, then create a named vector that maps each label to its numeric value. Let's check the labels in one commitment item:

```{r}
# Check what labels exist in a likert7 item
analytic_data_survey |>
  pull(aff_com1_likert7) |>
  unique()
```

Now we create a named vector that maps each text label to a number. The format `"Label" = number` tells R exactly what value each label should become:

```{r}
# Define the text-to-number mapping for 7-point scale
likert7_recode <- c(
  "Strongly Disagree" = 1,
...
Now we apply this mapping to all columns containing "likert7" in the name:
...
**How does `likert7_recode[.x]` work?** This uses R's named vector indexing. When you have a named vector like `c("a" = 1, "b" = 2)`, you can look up values by name: `vec["a"]` returns `1`. In our code, `.x` represents each cell's text value (e.g., "Strongly Agree"), and `likert7_recode[.x]` looks up the corresponding number (7). This is a concise and readable way to recode text to numbers.

You can see the columns are now numeric:

```{r}
analytic_data_survey |>
  select(contains("likert7")) |>
  glimpse()
```

##### Job Satisfaction Conversion

We follow the same process for the 5-point job satisfaction items. First, check the labels:

```{r}
# Check labels for a likert5 item
analytic_data_survey |>
  pull(job_aff1_likert5) |>
  unique()
```

Create the text-to-number mapping for the 5-point scale:

```{r}
# Define the text-to-number mapping for 5-point scale
likert5_recode <- c(
  "Strongly Disagree" = 1,
  "Disagree" = 2,
  "Neutral" = 3,
...
A complete discussion of the logic for flipping reverse-key (i.e., reverse worded) items is provide the previous chapter in the [Scale Scores](https://dstanley4.github.io/psyc6380/an-emphasis-on-workflow.html#scale-scores) section. We use that logic in the script below.
...
It's good practice to track how many participants are excluded so you can report this in your methods section. We do so with the code below:
...
The `nrow()` function returns the number of rows in a data frame. By saving the count before and after filtering, we can calculate exactly how many participants were excluded. The `message()` function prints informative text to the console—useful for documenting what happened during data processing.

```{r}
glimpse(analytic_data_survey)
```

This reduces the data set to the `r final_n` people who took at least 2 minutes (i.e., 120 seconds) to complete the survey.

##### Saving processed data

Once your data is prepared for analysis, save it using the `.rds` format. This preserves all R data types (factors, dates, etc.) exactly as they are—unlike CSV files which convert everything to text.
...
### Looking ahead: Modular scripts

In this chapter, we processed Qualtrics data using a single script. This approach works well for learning and for smaller projects. As projects grow more complex, however, it becomes helpful to break this single script into separate files—one for each major step:

```         
project/
├── 00-script-master.R      # Runs all scripts in order
├── 01-import.R             # Load and clean raw data
...
This modular approach has several benefits: easier debugging (you can run each step independently), better collaboration (team members can work on different sections), and clearer documentation (each file has one purpose). We'll explore this modular approach in a future lecture.
