...
└── output/                  # Folder: Output, tables, figures, etc.
```
...
::: {.callout-tip}
## Using AI is easier with this approach
The modular pipeline structure makes it easier to work with AI assistants like ChatGPT or Claude. You can share individual scripts for help
without overwhelming the AI with your entire project.
:::
...
```{r}
#| echo: false
missing_data_by_item <- read_csv(here("ch_data_pipeline/", "missing-data-by-item.csv"), show_col_types = FALSE)
...
```{r}
#| echo: false
missing_data_by_person <- read_csv(here("ch_data_pipeline/", "high-missing-participants.csv"), show_col_types = FALSE)
...
#| echo: false
#| output-width: "70%"
knitr::include_graphics(here("ch_data_pipeline/", "missing-data-map.png"))
```