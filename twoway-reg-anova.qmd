...
This example is from the Andy Field book. The example uses alcohol(0, 2 pints, 4 pints) and sex(male,female) as predictors of attractiveness. See the Discovering Statistics Using R (2012) for the complete example. We use this beer goggles example to illustrate sum contrasts in an ANOVA context.
...
But note that when we run the command above with gender\*alcohol we need to realize it is a shortcut convention for the code below - which implicitly includes an intercept:
...
2)  Multiple columns may be required for a single factor
...
It effectively runs the contr.sum() command on each factor column, when a regression is run, and creates contrasts based on the number of levels of each factor. For example, the sex factor, with two levels, causes the command below to be run.
...
In the output below, I have already applied this rule and put the result in the alc1 and alc2 columns. Normally this happens "under the hood" and you don't see it. Notice how all levels of alcohol are coded using this scheme; consistent with the contr.sum(3) command.
...
#| echo: false
#| 
gdata$intercept <- rep(1, 41)

gdata <- gdata %>%
  select(attractiveness, intercept, sex, alc1, alc2, int1, int2) %>% as.data.frame()
...
This is effectively identical to when we looked at comparing two regression models previously. Using that logic, we could just write the code:
...
# try this, it works!
library(apaTables)
apa.reg.table(lm_restricted_no_sex, lm_full)
```

The result would tell us if the main effect of sex is significant. The logic of calculating things this way is the Type III Sum of Squares logic. HOWEVER, we don't tend to use the output in this form. Rather the output is reformatted to by consistent with the way ANOVA's are typically presented. The next few sections show you how the table below is created:
